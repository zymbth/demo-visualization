...
function generateD3() {
  ...
  const yAxis = d3
    ...
    .tickSizeOuter(0)
  xAxisGroup = g.append('g').attr('transform', `translate(0, ${cellSize.width * rows})`)
  xAxisGroup
    .call(xAxis)
    .selectAll('text')
    .attr('width', 20)
...
function highlightCompoundCol(arr = highlightCompounds.value) {
  ...
  svgMainGroup
    ...
    .attr('x', (d, i) => xScale(d.idx) + cellSize.height / 2)
    .attr('y', 0 - cellSize.height / 2)
    .attr('width', cellSize.width)
    .attr('height', cellSize.width * (rows + 0.5))
